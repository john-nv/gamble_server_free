...
[DependsOn(
	typeof(GambleHttpApiClientModule),
...
public class GambleWebModule : AbpModule
{
	public override void PreConfigureServices(ServiceConfigurationContext context)
	{
		context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
		{
			options.AddAssemblyResource(
				typeof(GambleResource),
				typeof(GambleDomainSharedModule).Assembly,
				typeof(GambleApplicationContractsModule).Assembly,
				typeof(GambleWebModule).Assembly
			);
		});
	}
		...
		*/
		if (configuration.GetValue<bool>("AuthServer:IsContainerized"))
		{
			context.Services.Configure<OpenIdConnectOptions>("oidc", options =>
			{
				...
				{
					// Intercept the redirection for signout so the browser navigates to the right
					// URL in your host
	...
	private void ConfigureAutoMapper()
	{
		...
		{
			options.AddMaps<GambleWebModule>();
		});
	}
	...
	private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
	{
		if (hostingEnvironment.IsDevelopment())
		{
			Configure<AbpVirtualFileSystemOptions>(options =>
			{
				...
				options.FileSets.ReplaceEmbeddedByPhysical<GambleApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}OkVip.Gamble.Application.Contracts"));
				options.FileSets.ReplaceEmbeddedByPhysical<GambleWebModule>(hostingEnvironment.ContentRootPath);
			});
		}
	}
	...
	private void ConfigureNavigationServices(IConfiguration configuration)
	{
		...
		{
			options.MenuContributors.Add(new GambleMenuContributor(configuration));
		});

		Configure<AbpToolbarOptions>(options =>
		{
			options.Contributors.Add(new GambleToolbarContributor());
		});
	}
