...
namespace OkVip.Gamble.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
	...
	{
		if (await _openIddictScopeRepository.FindByNameAsync("Gamble") == null)
		{
			await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor
			{
				Name = "Gamble",
				DisplayName = "Gamble API",
				Resources = { "Gamble" }
			});
		}
	}
	...
	private async Task CreateApplicationsAsync()
	{
		...
		//Web Client
		var webClientId = configurationSection["Gamble_Web:ClientId"];
		if (!webClientId.IsNullOrWhiteSpace())
		{
			var webClientRootUrl = configurationSection["Gamble_Web:RootUrl"]!.EnsureEndsWith('/');

			/* Gamble_Web client is only needed if you created a tiered
			...
			await CreateApplicationAsync(
				...
				secret: configurationSection["Gamble_Web:ClientSecret"] ?? "1q2w3e*",
		...
		// Swagger Client
		var swaggerClientId = configurationSection["Gamble_Swagger:ClientId"];
		if (!swaggerClientId.IsNullOrWhiteSpace())
		{
			var swaggerRootUrl = configurationSection["Gamble_Swagger:RootUrl"]?.TrimEnd('/');
